...
def print_sign(integer):
    """prints if integer is higher than, lower than or equal to zero"""
    if integer > 0:
        print "Higher than 0"
    elif integer < 0:
        print "Lower than 0"
...
def is_divisible_by_three(integer):
    """returns True or False, if integer is divisible by 3"""
    if integer % 3 == 0:
        return True
    else:
        return False
...
def total_meal_price(price, tip=.15):
    """calculates price and tip of a meal"""
...
def sign_and_parity(integer):
    """determines if integer is positve/negative and even/odd"""
    if integer <= -1:
        sign = "Negative"
    else:
        sign = "Positive"
    ...
    sign_n_parity.append(sign)
    sign_n_parity.append(parity)

    return sign_n_parity
...
def full_title(name, job_title="Engineer"):
    """returns single string of a person's title and name"""

    return job_title + " " + name


def write_letter(name, job_title, sender_name):
    """writes letter to recipent. Says they're amazing."""
